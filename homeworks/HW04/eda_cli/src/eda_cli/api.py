...
class QualityFlagsResponse(BaseModel):
    ...
    flags: Dict[str, bool] = Field(
        ...,
        description="Полный набор булевых флагов качества из compute_quality_flags"
    )
...
@app.post(
    ...
    вычисленных функцией compute_quality_flags из EDA-ядра.
    Включает все флаги: булевые и числовые.
    """
)
async def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Эндпоинт, который принимает CSV-файл и возвращает
    полный набор флагов качества из compute_quality_flags.
    """
